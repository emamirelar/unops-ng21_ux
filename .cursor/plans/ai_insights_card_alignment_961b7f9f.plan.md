---
name: UX component alignment
overview: Replace local shims with real UX library components, align AI card behavior, upgrade documents-card to a shared library component with outputs for backend integration, and add Cursor rules to prevent future divergence.
todos:
  - id: replace-shim
    content: "Replace local ux-ai-card-bg shim: update tsconfig.json path alias, delete local shim files, update imports in all consuming files"
    status: pending
  - id: add-mdc-rule
    content: "Add rule to component-development.mdc: always import UX components from @emamirelar/ux, never create local shims"
    status: pending
  - id: collapse-header
    content: Add collapse/expand header row with chevron toggle and isAiCardExpanded signal to partner-view (and contact-view, opportunity-view)
    status: pending
  - id: insight-count
    content: Add subtitle showing total insight count below the AI card title
    status: pending
  - id: expand-body-wrap
    content: Wrap AI card body in .expand-body / .expand-body--open classes, default collapsed
    status: pending
  - id: remove-max-height
    content: Remove max-h-[min(24rem,50vh)] from insight list div so it fills available vertical space
    status: pending
  - id: verify-expand-css
    content: Verify .expand-body CSS in styles.scss includes opacity transition matching reference app
    status: pending
  - id: docs-card-upgrade
    content: "Upgrade documents-card: add output events (download, delete, upload, menuAction), loading input, read-only input, toolbar content projection slot, i18n support"
    status: pending
  - id: docs-card-publish
    content: "Move documents-card into the UX library (projects/unops-ux/src/lib/components/) and export from public-api"
    status: pending
  - id: docs-card-team-adopt
    content: "Document how the team replaces their app-document with app-documents-card: bind inputs/outputs, handle backend in parent"
    status: pending
isProject: false
---

# AI Insights Card — Align Dev with UX Reference
