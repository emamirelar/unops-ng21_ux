/**
 * @fileoverview PNO-1156 test fixture base — Responsible Org Unit in Create Opportunity from Interactions dialog.
 * Provides in-memory UNOPSAppDbContext, seeded reference data (OrgUnit, Partner, Interaction, Currency, etc.),
 * UNOPSOpportunityManager, and helpers for CreateOpportunityFromProposalAsync tests.
...
/// <summary>
/// Base fixture for PNO-1156 tests.
/// PNO-1156: Add Responsible Org Unit dropdown to Create Opportunity from Interactions dialog.
/// Tests UNOPSOpportunityManager.CreateOpportunityFromProposalAsync with ResponsibleOrgUnitId.
/// </summary>
public abstract class PNO1156TestFixtureBase : IDisposable
{
    ...
    protected PNO1156TestFixtureBase()
    {
    ...
    protected async Task<Interaction> SeedInteractionAsync(int id, string subject, int? partnerId = null)
    {
    ...
    protected CreateOpportunityFromInteractionsRequest BuildRequest(
        string name = "PNO-1156 Test Opportunity",
    ...
    protected async Task VerifyOpportunityHasOrgUnitAsync(int opportunityId, int expectedOrgUnitId)
    {
        var opp = await GetOpportunityFromDbAsync(opportunityId);
        opp.Should().NotBeNull();
        opp!.ResponsibleOrgUnitId.Should().Be(expectedOrgUnitId);
    }
