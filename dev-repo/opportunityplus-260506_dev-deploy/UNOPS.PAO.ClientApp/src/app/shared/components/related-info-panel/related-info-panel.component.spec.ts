  ...
  template: `
    <div class="related-info-panel" [class.collapsed]="collapsed()">
      <header class="panel-header" (click)="toggleCollapse()">
        ...
        <span class="panel-count" *ngIf="items().length > 0">({{ items().length }})</span>
        <button class="add-button" *ngIf="showAddButton()" (click)="onAddClick($event)">+</button>
      </header>
      <div class="panel-body" *ngIf="!collapsed()">
        <div class="loading-skeleton" *ngIf="loading()">Loading...</div>
        ...
        <ul class="item-list" *ngIf="!loading() && !error() && items().length > 0">
          <li 
            *ngFor="let item of displayedItems()" 
            class="panel-item"
            (click)="onItemClick(item)">
            <span class="item-name">{{ item.name }}</span>
            <span class="item-detail" *ngIf="item.detail">{{ item.detail }}</span>
        ...
        <a class="see-all-link" *ngIf="showSeeAll() && items().length > maxItems()" (click)="onSeeAllClick()">
