/**
 * Action handler for a partner chip being clicked.
 * This function retrieves the partner JSON string passed as a parameter.
 *
 ...
 */
function onPartnerChipSelected(e) {
  const partner = e.parameters ? JSON.parse(e.parameters.partner) : undefined;
  ...
  if(partner.website) {
    detailSection.addWidget(
            CardService.newDecoratedText()
              .setStartIcon(CardService.newIconImage().setMaterialIcon(CardService.newMaterialIcon().setName('web')))
              .setText(partner.phone)
              .setBottomLabel('Website')
          );
  }
  ...
  if(partner.location) {
    detailSection.addWidget(
            CardService.newDecoratedText()
              ...
              .setText(partner.location)
              .setBottomLabel('Address1')
          );
  }

  if(partner.phone || partner.website || partner.location) {
        ...
        if(contact.canRead) {
          contactSection.addWidget(
            CardService.newDecoratedText()
              ...
              .setBottomLabel(contact.emailAddress)
          );
        }
        else {
