...
{
    public static class Partners_Related_To_Level4_Bulk_Update
    {
        public static async Task UpdatePartnersForIntegrationAsync(UNOPSAppDbContext context)
        {
            Console.WriteLine("Starting Partners Related To Level 4 bulk update - Setting LastModifiedBy and LastModifiedDate...");
            ...
            try
            {
                ...
                // Record 217: ErpDimValue = 1554
                {
                    var existingPartner = await context.Partners
                        .FirstOrDefaultAsync(p => p.ErpDimValue == 1554);
                    
                    if (existingPartner != null)
                    {
                        updatedRecordIds.Add(existingPartner.Id);
                        Console.WriteLine($"Found: Partner with ErpDimValue '1554' - {existingPartner.Name}");
                        updatedCount++;
                    }
                    else
                    {
                        Console.WriteLine($"Not Found: Partner with ErpDimValue '1554' does not exist.");
                        notFoundCount++;
                    }
                }
                
                // Record 218: ErpDimValue = 1564
                {
                    var existingPartner = await context.Partners
                        .FirstOrDefaultAsync(p => p.ErpDimValue == 1564);
                    
                    if (existingPartner != null)
                    {
                        updatedRecordIds.Add(existingPartner.Id);
                        Console.WriteLine($"Found: Partner with ErpDimValue '1564' - {existingPartner.Name}");
                        updatedCount++;
                    }
                    else
                    {
                        Console.WriteLine($"Not Found: Partner with ErpDimValue '1564' does not exist.");
                        notFoundCount++;
                    }
                }
                
                // Record 219: ErpDimValue = 1557
                {
                    var existingPartner = await context.Partners
                        .FirstOrDefaultAsync(p => p.ErpDimValue == 1557);
                    
                    if (existingPartner != null)
                    {
                        updatedRecordIds.Add(existingPartner.Id);
                        Console.WriteLine($"Found: Partner with ErpDimValue '1557' - {existingPartner.Name}");
                        updatedCount++;
                    }
                    else
                    {
                        Console.WriteLine($"Not Found: Partner with ErpDimValue '1557' does not exist.");
                        notFoundCount++;
                    }
                }
                
                // Record 220: ErpDimValue = 1556
                {
                    var existingPartner = await context.Partners
                        .FirstOrDefaultAsync(p => p.ErpDimValue == 1556);
                    
                    if (existingPartner != null)
                    {
                        updatedRecordIds.Add(existingPartner.Id);
                        Console.WriteLine($"Found: Partner with ErpDimValue '1556' - {existingPartner.Name}");
                        updatedCount++;
                    }
                    else
                    {
                        Console.WriteLine($"Not Found: Partner with ErpDimValue '1556' does not exist.");
                        notFoundCount++;
                    }
                }
                
                // Record 221: ErpDimValue = 1560
                {
                    var existingPartner = await context.Partners
                        .FirstOrDefaultAsync(p => p.ErpDimValue == 1560);
                    
                    if (existingPartner != null)
                    {
                        updatedRecordIds.Add(existingPartner.Id);
                        Console.WriteLine($"Found: Partner with ErpDimValue '1560' - {existingPartner.Name}");
                        updatedCount++;
                    }
                    else
                    {
                        Console.WriteLine($"Not Found: Partner with ErpDimValue '1560' does not exist.");
                        notFoundCount++;
                    }
                }
                
                // Record 222: ErpDimValue = 1489
                {
                    var existingPartner = await context.Partners
                ...
                // Record 223: ErpDimValue = 1551
                {
                    var existingPartner = await context.Partners
                        .FirstOrDefaultAsync(p => p.ErpDimValue == 1551);
                    
                    if (existingPartner != null)
                    {
                        updatedRecordIds.Add(existingPartner.Id);
                        Console.WriteLine($"Found: Partner with ErpDimValue '1551' - {existingPartner.Name}");
                        updatedCount++;
                    }
                    else
                    {
                        Console.WriteLine($"Not Found: Partner with ErpDimValue '1551' does not exist.");
                        notFoundCount++;
                    }
                }
                
                // Record 224: ErpDimValue = 1559
                {
                    var existingPartner = await context.Partners
                        .FirstOrDefaultAsync(p => p.ErpDimValue == 1559);
                    
                    if (existingPartner != null)
                    {
                        updatedRecordIds.Add(existingPartner.Id);
                        Console.WriteLine($"Found: Partner with ErpDimValue '1559' - {existingPartner.Name}");
                        updatedCount++;
                    }
                    else
                    {
                        Console.WriteLine($"Not Found: Partner with ErpDimValue '1559' does not exist.");
                        notFoundCount++;
                    }
                }
                
                // Record 225: ErpDimValue = 1562
                {
                    var existingPartner = await context.Partners
                        .FirstOrDefaultAsync(p => p.ErpDimValue == 1562);
                    
                    if (existingPartner != null)
                    {
                        updatedRecordIds.Add(existingPartner.Id);
                        Console.WriteLine($"Found: Partner with ErpDimValue '1562' - {existingPartner.Name}");
                        updatedCount++;
                    }
                    else
                    {
                        Console.WriteLine($"Not Found: Partner with ErpDimValue '1562' does not exist.");
                        notFoundCount++;
                    }
                }
                
                // Record 226: ErpDimValue = 1548
                {
                    var existingPartner = await context.Partners
                        .FirstOrDefaultAsync(p => p.ErpDimValue == 1548);
                    
                    if (existingPartner != null)
                    {
                        updatedRecordIds.Add(existingPartner.Id);
                        Console.WriteLine($"Found: Partner with ErpDimValue '1548' - {existingPartner.Name}");
                        updatedCount++;
                    }
                    else
                    {
                        Console.WriteLine($"Not Found: Partner with ErpDimValue '1548' does not exist.");
                        notFoundCount++;
                    }
                }
                
                // Record 227: ErpDimValue = 1561
                {
                    var existingPartner = await context.Partners
                        .FirstOrDefaultAsync(p => p.ErpDimValue == 1561);
                    
                    if (existingPartner != null)
                    {
                        updatedRecordIds.Add(existingPartner.Id);
                        Console.WriteLine($"Found: Partner with ErpDimValue '1561' - {existingPartner.Name}");
                        updatedCount++;
                    }
                    else
                    {
                        Console.WriteLine($"Not Found: Partner with ErpDimValue '1561' does not exist.");
                        notFoundCount++;
                    }
                }
                
                // Record 228: ErpDimValue = 1553
                {
                    var existingPartner = await context.Partners
                        .FirstOrDefaultAsync(p => p.ErpDimValue == 1553);
                    
                    if (existingPartner != null)
                    {
                        updatedRecordIds.Add(existingPartner.Id);
                        Console.WriteLine($"Found: Partner with ErpDimValue '1553' - {existingPartner.Name}");
                        updatedCount++;
                    }
                    else
                    {
                        Console.WriteLine($"Not Found: Partner with ErpDimValue '1553' does not exist.");
                        notFoundCount++;
                    }
                }
                
                // Record 229: ErpDimValue = 1191
                {
                    var existingPartner = await context.Partners
                        .FirstOrDefaultAsync(p => p.ErpDimValue == 1191);
                    
                    if (existingPartner != null)
                    {
                        updatedRecordIds.Add(existingPartner.Id);
                        Console.WriteLine($"Found: Partner with ErpDimValue '1191' - {existingPartner.Name}");
                        updatedCount++;
                    }
                    else
                    {
                        Console.WriteLine($"Not Found: Partner with ErpDimValue '1191' does not exist.");
                        notFoundCount++;
                    }
                }
                
                // Record 230: ErpDimValue = 1775
                {
                    var existingPartner = await context.Partners
                        .FirstOrDefaultAsync(p => p.ErpDimValue == 1775);
                    
                    if (existingPartner != null)
                    {
                        updatedRecordIds.Add(existingPartner.Id);
                        Console.WriteLine($"Found: Partner with ErpDimValue '1775' - {existingPartner.Name}");
                        updatedCount++;
                    }
                    else
                    {
                        Console.WriteLine($"Not Found: Partner with ErpDimValue '1775' does not exist.");
                        notFoundCount++;
                    }
                }
                
                // Record 231: ErpDimValue = 1570
                {
                    var existingPartner = await context.Partners
                        .FirstOrDefaultAsync(p => p.ErpDimValue == 1570);
                    
                    if (existingPartner != null)
                    {
                        updatedRecordIds.Add(existingPartner.Id);
                        Console.WriteLine($"Found: Partner with ErpDimValue '1570' - {existingPartner.Name}");
                        updatedCount++;
                    }
                    else
                    {
                        Console.WriteLine($"Not Found: Partner with ErpDimValue '1570' does not exist.");
                        notFoundCount++;
                    }
                }
                
                // Record 232: ErpDimValue = 1552
                {
                    var existingPartner = await context.Partners
                        .FirstOrDefaultAsync(p => p.ErpDimValue == 1552);
                    
                    if (existingPartner != null)
                    {
                        updatedRecordIds.Add(existingPartner.Id);
                        Console.WriteLine($"Found: Partner with ErpDimValue '1552' - {existingPartner.Name}");
                        updatedCount++;
                    }
                    else
                    {
                        Console.WriteLine($"Not Found: Partner with ErpDimValue '1552' does not exist.");
                        notFoundCount++;
                    }
                }
                
                // Record 233: ErpDimValue = 1189
                {
                    var existingPartner = await context.Partners
                        .FirstOrDefaultAsync(p => p.ErpDimValue == 1189);
                    
                    if (existingPartner != null)
                    {
                        updatedRecordIds.Add(existingPartner.Id);
                        Console.WriteLine($"Found: Partner with ErpDimValue '1189' - {existingPartner.Name}");
                        updatedCount++;
                    }
                    else
                    {
                        Console.WriteLine($"Not Found: Partner with ErpDimValue '1189' does not exist.");
                        notFoundCount++;
                    }
                }
                
                // Record 234: ErpDimValue = 1188
                {
                    var existingPartner = await context.Partners
                        .FirstOrDefaultAsync(p => p.ErpDimValue == 1188);
                    
                    if (existingPartner != null)
                    {
                        updatedRecordIds.Add(existingPartner.Id);
                        Console.WriteLine($"Found: Partner with ErpDimValue '1188' - {existingPartner.Name}");
                        updatedCount++;
                    }
                    else
                    {
                        Console.WriteLine($"Not Found: Partner with ErpDimValue '1188' does not exist.");
                        notFoundCount++;
                    }
                }
                
                // Record 235: ErpDimValue = 1190
                {
                    var existingPartner = await context.Partners
                        .FirstOrDefaultAsync(p => p.ErpDimValue == 1190);
                    
                    if (existingPartner != null)
                    {
                        updatedRecordIds.Add(existingPartner.Id);
                        Console.WriteLine($"Found: Partner with ErpDimValue '1190' - {existingPartner.Name}");
                        updatedCount++;
                    }
                    else
                    {
                        Console.WriteLine($"Not Found: Partner with ErpDimValue '1190' does not exist.");
                        notFoundCount++;
                    }
                }
                
                // Record 236: ErpDimValue = 1565
                {
                    var existingPartner = await context.Partners
                        .FirstOrDefaultAsync(p => p.ErpDimValue == 1565);
                    
                    if (existingPartner != null)
                    {
                        updatedRecordIds.Add(existingPartner.Id);
                        Console.WriteLine($"Found: Partner with ErpDimValue '1565' - {existingPartner.Name}");
                        updatedCount++;
                    }
                    else
                    {
                        Console.WriteLine($"Not Found: Partner with ErpDimValue '1565' does not exist.");
                        notFoundCount++;
                    }
                }
                
                // Record 237: ErpDimValue = 1563
                {
                    var existingPartner = await context.Partners
                        .FirstOrDefaultAsync(p => p.ErpDimValue == 1563);
                    
                    if (existingPartner != null)
                    {
                        updatedRecordIds.Add(existingPartner.Id);
                        Console.WriteLine($"Found: Partner with ErpDimValue '1563' - {existingPartner.Name}");
                        updatedCount++;
                    }
                    else
                    {
                        Console.WriteLine($"Not Found: Partner with ErpDimValue '1563' does not exist.");
                        notFoundCount++;
                    }
                }
                
                // Record 238: ErpDimValue = 1549
                {
                    var existingPartner = await context.Partners
                        .FirstOrDefaultAsync(p => p.ErpDimValue == 1549);
                    
                    if (existingPartner != null)
                    {
                        updatedRecordIds.Add(existingPartner.Id);
                        Console.WriteLine($"Found: Partner with ErpDimValue '1549' - {existingPartner.Name}");
                        updatedCount++;
                    }
                    else
                    {
                        Console.WriteLine($"Not Found: Partner with ErpDimValue '1549' does not exist.");
                        notFoundCount++;
                    }
                }
                
                // Record 239: ErpDimValue = 1555
                {
                    var existingPartner = await context.Partners
                        .FirstOrDefaultAsync(p => p.ErpDimValue == 1555);
                    
                    if (existingPartner != null)
                    {
                        updatedRecordIds.Add(existingPartner.Id);
                        Console.WriteLine($"Found: Partner with ErpDimValue '1555' - {existingPartner.Name}");
                        updatedCount++;
                    }
                    else
                    {
                        Console.WriteLine($"Not Found: Partner with ErpDimValue '1555' does not exist.");
                        notFoundCount++;
                    }
                }
                
                // Record 240: ErpDimValue = 1558
                {
                    var existingPartner = await context.Partners
                        .FirstOrDefaultAsync(p => p.ErpDimValue == 1558);
                    
                    if (existingPartner != null)
                    {
                        updatedRecordIds.Add(existingPartner.Id);
                        Console.WriteLine($"Found: Partner with ErpDimValue '1558' - {existingPartner.Name}");
                        updatedCount++;
                    }
                    else
                    {
                        Console.WriteLine($"Not Found: Partner with ErpDimValue '1558' does not exist.");
                        notFoundCount++;
                    }
                }
                
                // Record 241: ErpDimValue = 1651
                {
                    var existingPartner = await context.Partners
                        .FirstOrDefaultAsync(p => p.ErpDimValue == 1651);
                    
                    if (existingPartner != null)
                    {
                        updatedRecordIds.Add(existingPartner.Id);
                        Console.WriteLine($"Found: Partner with ErpDimValue '1651' - {existingPartner.Name}");
                        updatedCount++;
                    }
                    else
                    {
                        Console.WriteLine($"Not Found: Partner with ErpDimValue '1651' does not exist.");
                        notFoundCount++;
                    }
                }
                
                // Record 242: ErpDimValue = 1566
                {
                    var existingPartner = await context.Partners
                        .FirstOrDefaultAsync(p => p.ErpDimValue == 1566);
                    
                    if (existingPartner != null)
                    {
                        updatedRecordIds.Add(existingPartner.Id);
                        Console.WriteLine($"Found: Partner with ErpDimValue '1566' - {existingPartner.Name}");
                        updatedCount++;
                    }
                    else
                    {
                        Console.WriteLine($"Not Found: Partner with ErpDimValue '1566' does not exist.");
                        notFoundCount++;
                    }
                }
                
                // Record 243: ErpDimValue = 1550
                {
                    var existingPartner = await context.Partners
                        .FirstOrDefaultAsync(p => p.ErpDimValue == 1550);
                    
                    if (existingPartner != null)
                    {
                        updatedRecordIds.Add(existingPartner.Id);
                        Console.WriteLine($"Found: Partner with ErpDimValue '1550' - {existingPartner.Name}");
                        updatedCount++;
                    }
                    else
                    {
                        Console.WriteLine($"Not Found: Partner with ErpDimValue '1550' does not exist.");
                        notFoundCount++;
                    }
                }
                
                // Record 244: ErpDimValue = 1187
                {
                    var existingPartner = await context.Partners
                        .FirstOrDefaultAsync(p => p.ErpDimValue == 1187);
                    
                    if (existingPartner != null)
                    {
                        updatedRecordIds.Add(existingPartner.Id);
                        Console.WriteLine($"Found: Partner with ErpDimValue '1187' - {existingPartner.Name}");
                        updatedCount++;
                    }
                    else
                    {
                        Console.WriteLine($"Not Found: Partner with ErpDimValue '1187' does not exist.");
                        notFoundCount++;
                    }
                }
                
                // Record 245: ErpDimValue = 1590
                {
                    var existingPartner = await context.Partners
                ...
                Console.WriteLine($"\nPartners Related To Level 4 bulk update completed successfully.");
            ...
            catch (Exception ex)
            {
                ...
                Console.WriteLine($"Error during Partners Related To Level 4 bulk update: {ex.Message}");
