namespace UNOPS.PAO.Utilities.Registers;
...
public abstract class Register<T>
{
    ...
    protected Register(DependencyInjectionContainer container)
    {
    ...
    /// </summary>
    /// <param name="key">
    ///     Name of the type to instantiate. This should correspond to the
    ///     value stored in <see cref="RegisterEntryAttribute.Key" />, which decorates the type.
    /// </param>
    ...
    public T GetInstance(string key)
    {
        ...
        throw new ApplicationException($"Type '{key}' is not registered with '{GetType().FullName}'.");
    }
    ...
    {
        foreach (var key in registeredTypes.Values)
        {
            yield return GetInstance(key);
        }
    }
    ...
    {
        var typesToRegister = assembly.GetLoadableTypes()
