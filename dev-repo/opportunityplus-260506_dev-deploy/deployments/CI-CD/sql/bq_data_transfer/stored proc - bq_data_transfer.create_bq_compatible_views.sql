CREATE OR REPLACE PROCEDURE bq_data_transfer.create_bq_compatible_views()
 LANGUAGE plpgsql
AS $procedure$
DECLARE
  tbl_name text; view_name text; dynamic_select_cols text; create_view_sql text;
BEGIN
  ...
  FOR tbl_name IN 
      ...
      view_name := tbl_name; -- Generate the safe, BQ-compliant column selection SQL for the current table 
      SELECT 
            ...
            STRING_AGG( 
              CASE 
                ...
                'CASE WHEN ' || quote_ident(column_name) || ' IS NULL THEN NULL WHEN NOT isfinite(' 
                    ...
                    || ') THEN NULL WHEN ' 
                    || quote_ident(column_name) 
                    || ' < ''1970-01-01 00:00:00''::timestamptz THEN NULL ELSE DATE_TRUNC(''second'', ' 
                    || quote_ident(column_name) 
                    || ') END AS ' 
                    || quote_ident(column_name) 
      ...
      THEN -- Assemble the final CREATE VIEW statement 
      create_view_sql := 'CREATE OR REPLACE VIEW bq_data_transfer.' || quote_ident(view_name) 
                          || ' AS SELECT ' || dynamic_select_cols 
                          || ' FROM public.' || quote_ident(tbl_name) 
                          || ';'; 

              -- Execute the DDL statement 
              EXECUTE create_view_sql; 
              RAISE NOTICE 'Created or replaced view: %', view_name; 
      ELSE 
      RAISE NOTICE 'Skipping table % (no columns found or only unsupported types)', tbl_name; 
      END IF; 
      END LOOP; 
END; 
$procedure$