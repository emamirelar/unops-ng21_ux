...
def generate_interaction_from_task_seeder_v3(csv_file_path: str, output_file_path: str) -> None:
    ...
    for task in tasks:
        ...
        task_id = clean_field(task.get('Id', ''))
        
        # Handle missing subject and description
        if not subject and not description:
            # Use defaults with Id if both are missing
            subject = f"NO_SUB{task_id}" if task_id else "NO_SUBJECT"
            description = f"NO_DESC{task_id}" if task_id else "NO_DESCRIPTION"
        elif not subject:
        ...
        # New interaction
        gmail_message_id = task_id  # Already retrieved earlier
        ...
        # Parse ActivityDate for interaction date
        ...
        created_date_csv = clean_field(task.get('CreatedDate', ''))
        created_date_str = "DateTime.UtcNow"
        if created_date_csv:
            try:
                # Handle ISO format with milliseconds: 2024-10-09T16:31:48.000+0000
                datetime_part = created_date_csv.split('.')[0]
    ...
    print(f"No records skipped - all tasks processed")
...
{{
    public static class InteractionFromTaskSeeder_v3
    {{
...
{chr(10).join(interaction_tuples)}
    ...
    try:
        ...
        print(f"Successfully generated InteractionFromTaskSeeder_v3.cs")
...
def main():
    ...
    csv_file = os.path.join(script_dir, "Task-11_4_2025 - Task-11_4_2025.csv")
    output_file = os.path.join(script_dir, "..", "..", "SeederFiles", "InteractionFromTaskSeeder_v3.cs")
    
    print("InteractionFromTaskSeeder_v3 Generator")
    print("=" * 50)
    ...
    # Generate the seeder
    generate_interaction_from_task_seeder_v3(csv_file, output_file)
