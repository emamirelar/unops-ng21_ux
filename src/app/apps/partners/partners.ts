        ...
        <div class="flex flex-col gap-6">
            ...
            <div class="flex flex-col gap-3">
                <p-iconfield class="w-full sm:w-72">
                    ...
                    <input pInputText [ngModel]="searchQuery()" (ngModelChange)="searchQuery.set($event)" placeholder="Search partners..." class="w-full" />
                </p-iconfield>

                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
                    <div class="flex items-center flex-wrap gap-2">
                        <p-tag [value]="filteredPartners().length + ' Partners'" icon="pi pi-users" severity="secondary" />
                        <p-tag [value]="activeCount() + ' Active'" icon="pi pi-check-circle" severity="secondary" />
                        <p-tag [value]="keyGlobalCount() + ' Key Global'" icon="pi pi-star" severity="secondary" />
                    </div>
            ...
            <p-dataview [value]="filteredPartners()" [layout]="layout" [pt]="{ header: { class: 'hidden' } }">
                    ...
                    <div class="flex flex-col">
                        @for (item of items; track item.id; let i = $index) {
                            <a
                                ...
                                class="flex flex-row items-start sm:items-center px-0 sm:px-4 py-4 gap-4 no-underline text-inherit cursor-pointer hover:bg-primary-100 dark:hover:bg-primary-900 transition-colors animate-fade-in-up"
                                ...
                                <div class="flex flex-col md:flex-row justify-between md:items-center flex-1 gap-4">
                                    <div class="flex flex-col gap-1 min-w-0">
                                        <div class="flex items-center gap-2 flex-wrap min-w-0">
                                            <div class="flex items-center justify-center w-8 h-8 rounded-lg bg-primary/10 shrink-0 overflow-hidden">
                                                <img [src]="getFlagUrl(item)" [alt]="item.address1Country || 'Global'" class="w-6 h-6 object-contain" />
                                            </div>
                                            <span class="text-surface-900 dark:text-surface-0 text-base font-semibold break-words min-w-0">{{ item.shortName || item.name }}</span>
                                            @if (item.shortName) {
                                                <span class="text-surface-600 dark:text-surface-300 text-sm truncate min-w-0">{{ item.name }}</span>
                                            }
                                            @if (item.keyGlobalPartner) {
                                                <i class="pi pi-star-fill text-amber-500 text-xs shrink-0" title="Key Global Partner"></i>
                                            }
